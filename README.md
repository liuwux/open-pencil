...
**Read-only file session** (no desktop app):

```json
{
  "mcpServers": {
    "open-pencil-readonly": {
      "command": "openpencil-mcp-headless",
      "args": ["/path/to/design.fig"]
    }
  }
}
```

The fixed input file is decoded lazily on the first tool call and its SceneGraph is reused for
the lifetime of the MCP process. This entry point exposes inspection tools only and never writes
the source file.
